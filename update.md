...
| Date | What | Commit |
|---|---|---|
| 2026-08-25 | **HR masters made per-tenant; batch/stock writes made atomic.** Two audit items, verified against the code before acting — one was mis-scoped and one was largely already done. **(1) Cross-tenant leak in HR masters (the real P0).** `Department`, `Designation` and `LeaveType` had **no `organizationId` at all** — global tables like `Role`/`VoucherType`. `leave-types` GET was `where: includeInactive ? {} : { isActive: true }` — *no scoping whatsoever*, so every org read every other org's leave catalogue. `departments` GET scoped via `employees: { some: { organizationId } }` **OR'd with `{ employees: { none: {} } }`**, leaking every not-yet-staffed department into every tenant and hiding the evidence once somebody was assigned. Both code-clash checks (`departments` POST + PATCH, `leave-types` POST) queried `where: { code }` unscoped, so tenant B was refused a code because tenant A held it — disclosure plus a functional block. `leaves` POST resolved `leaveTypeId` by bare id. **Migration 17** adds the column to all three, **clones** the shared rows into every organization (re-pointing `employees.departmentId`/`.designationId` and `leaves.leaveTypeId` at each org's own copy, de-duped on `lower(name)`) rather than handing the shared set to whoever touched it first, then adds `@@unique([organizationId, name])` (+ `code` where applicable), the FKs and indexes. Adding a *global* `@@unique` — what the punch list proposed — would have made this worse, permanently blocking tenant B from a name tenant A used. HR master creation moved into `provisionOrganization` (seed/register/`POST /api/organizations` all bottom out there); the three global `upsert` blocks deleted from `prisma/seed.ts`. **(2) `inventory/batches` — no transaction, no audit.** POST did `batch.create` then `stock.upsert`; PATCH did `batch.update` then `stock.update`; a failure between them left a batch nobody could account for and a warehouse short by exactly its quantity. Both wrapped in `$transaction` with `writeAudit` inside. `stock.upsert` replaced with a raw `INSERT ... ON CONFLICT` — `posting.ts` already documents that Prisma's upsert resolves as select-then-insert under the pg driver adapter and loses the race; here that meant concurrent batches of the same item took each other down. P2002 now answers **409** via a new `conflict()` helper in `with-org-auth.ts`, not 500. **(3) Cron was already done** — `tests/integration/cron.test.ts` already covers authorized 200, wrong token, unauthenticated, GET+POST and inactive-org skip, in CI. Only remaining step is setting `CRON_SECRET` on Vercel. **(4) TOCTOU sweep, counted honestly:** 34 check-then-create pairs across `src/app/api`, **29 constraint-backed**, **5 not** — `GSTReturn` (the one that matters — duplicate period rows), `Leave`, `Role`, `BankReconciliation`, `ReportTemplate`. Not fixed; each needs its own natural key decided. +16 integration tests (`hr-masters-tenancy.test.ts`, `batch-stock-atomicity.test.ts`). **624 unit + 107 integration + 27 e2e green**, tsc + lint clean (0 errors), migration 17 applies clean. | _(working tree)_ |
| 2026-08-23 | **Document inbox — OCR extraction, phase 1 (upload → read → two-panel review → post).** New `/documents` (queue + drag-drop upload + 30-day extraction spend) and `/documents/[id]` (original on the left with zoom/rotate, editable invoice/bill form on the right, low-confidence fields ringed amber, autosave, totals-don't-tie warning, confirm → posts a **draft** Bill or Invoice). **Migration 16** adds `document_extractions` — the file, the reading, the reviewer's corrected version, and the per-document cost (`provider`/`model`/tokens/`costMicroUsd`), because extraction is the first thing here with a real marginal cost and packs of credits have to be priced off recorded spend, not guesses. **Two engines, free-first:** `services/ocr/providers/pdf-text.ts` (unpdf + `heuristics.ts` — GSTIN/invoice-no/day-first-date/₹-grouping regexes, ~₹0/page, works on vendor-generated PDFs) and `providers/claude.ts` (vision + structured outputs against `EXTRACTION_JSON_SCHEMA`, handles photos/scans/handwriting, ~$0.04/page on `claude-opus-5`, `effort: low`). `extract.ts` escalates free → paid only when the free reading comes back thin; `OCR_ENGINE=auto\|free\|claude\|manual` and `OCR_MODEL` override. No key configured ⇒ the inbox still works as side-by-side manual entry. Storage: `services/documents/storage.ts` — Vercel Blob when `BLOB_READ_WRITE_TOKEN` is set, `./.uploads` locally; files are served through an org-scoped route, never a public blob URL. `services/documents/post-extracted.ts` posts the confirmed reading: party matched by GSTIN → name → created, **the document's own tax split wins** over what our state data implies, and a stated total that doesn't equal lines+tax is carried as `roundOff` rather than argued with. Deliberately drafts, not postings — a machine-read purchase does not skip approval. **Not built yet: email ingestion** (IMAP sweep of a mailbox configured in Settings) — that's phase 2; the schema already carries `source`/`sourceRef` for it. +17 unit tests, +10 integration tests. **618 unit + 91 integration green**, tsc + lint clean, `next build` ✓, migration 16 applies clean with no drift. New deps: `@anthropic-ai/sdk`, `@vercel/blob`, `unpdf`. | _(working tree)_ |
| 2026-08-23 | **Invoice pops up from `/inventory/stock`.** In-progress panel rows used to only hand off to the dispatch queue; each row now has **View invoice** (+ a **View invoice** button on every dispatch-queue invoice header) which opens the real tax invoice in a dialog over the pick list. The invoice layout was extracted out of `sales/invoices/[invoiceId]/page.tsx` into `frontend/components/features/sales/invoice-document.tsx` (pure presentation, no fetching/chrome) and reused by the new `invoice-preview-dialog.tsx` (fetches invoice + org, Print opens `/sales/invoices/<id>?print=1` in a new tab since the page's print stylesheet already handles A4, plus "Open full invoice"). Detail page now renders `<InvoiceDocument>` — 463 → 121 lines, no visual change. **Plus "mark invoice complete"** for the warehouse manager: `planInvoiceDispatch` + `completeInvoiceDispatch` in `services/inventory/dispatch.ts` allocate every still-pending line to a warehouse that actually holds the goods (preferred → default → largest, splitting across warehouses when no single one covers a line) and then go through the existing `postDispatch` transaction, so the concurrency guard and pending re-derivation are unchanged. Refuses outright on a shortfall rather than half-shipping an invoice it then calls complete. New route `POST /stock/dispatch/complete` with `preview: true` returning the plan, so `CompleteInvoiceDialog` shows the server's own allocation (per-warehouse pick list, on-hand → after) and confirming posts exactly that. Triggers: in-progress sheet row, dispatch-queue invoice header (replaced "Confirm all", which the checkbox bulk action already covered), and the invoice popup. "Complete" stays **derived** — no new column: a fully-dispatched invoice simply has no pending lines left, matching how the whole three-position model works. +7 integration tests (`tests/integration/dispatch-complete.test.ts`). **596 unit + 81 integration green**, tsc + lint clean. | _(working tree)_ |
| 2026-08-22 | **Phase punch-list #1–#10 complete.** Continuing the entry below. **#6 cron:** auth was already sound (fail-closed on unset secret, constant-time compare) but wholly untested — added 11 unit tests (unset secret ⇒ 503, malformed/missing/wrong/prefix/suffix tokens ⇒ 401) + 8 integration tests proving both crons actually sweep every *active* org with a valid token and 401 without, and that both export GET **and** POST (Vercel Cron issues GET). **#9 retrofit:** Ledger Export Scope moved off mocked Prisma to real two-tenant data — org B passing org A's `ledgerId` now provably gets 404 with none of A's data in the body; TDS/TCS "YTD scope" turned out to be pure calc tests, so the real gap was the *aggregate queries* — added integration tests that prior-FY, other-org, other-party and non-COMPLETED rows are all excluded from the threshold. **#8 pages:** Notifications / Profile / Custom Reports verified (real fetches + loading/error/empty). Taxation Reports was a genuine gap — a static menu with a fake loading state; now fetches gst-returns + tds + tcs and shows a live count per card, with error and retry states. **#10 verification:** tsc clean · lint 0 errors (19 pre-existing warnings) · **567 unit + 44 integration green** · production build ✓ · all 16 migrations apply clean on a from-scratch database with no drift · no unintended files (only the pre-existing untracked `docs/AccuBook-Launch.*`). **SHIPPED** as `81862c7` — production migrated `15_org_payroll_settings` (one additive nullable JSONB column) and is serving; `/api/health` reports 16/16 migrations, no drift. Merged with Achyut's 9 commits on main: kept his `support/guard.ts` allow-list + `isPosted: true` report filters, kept my `setupFiles` + `TEST_DATABASE_URL` override + CI wiring; `harness.test.ts` loopback assertion widened to private ranges so a containerised Postgres (the only practical CI option) passes. **Note for future-me:** `vitest.integration.config.ts` pins `DATABASE_URL` to `TEST_DATABASE_URL` *before app modules load* — `src/config/env.ts` freezes it at import and `.env` points at Neon **production**, so never run the integration suite without that pin. | `81862c7` |
| 2026-08-21 | **Three-position stock model + phase punch-list (#1–#10), part 1.** UI: `/inventory/stock` rebuilt around Accounting / In Progress / Physical (`physical = accounting + inProgress`), segmented split bar, clickable In-Progress → right slide-over listing the invoices behind it, new **Dispatch Queue** tab (pick list by invoice, per-line partial qty, ship-from warehouse chosen at pick time because *no sales document carries a warehouseId*), confirm dialog showing the before/after on all three buckets. Dispatch posts for real: `POST /stock/dispatch` → `services/inventory/dispatch.ts` re-derives pending **inside the transaction** and decrements with a `quantity >= qty` guard (proven by a concurrency test). **#1 costing:** decided **periodic with derived closing stock** (Tally-style; bills already `Dr Purchase Accounts`, so no historical restatement). New `services/inventory/valuation.ts` — honours `Item.valuationMethod` (FIFO/LIFO/weighted-avg) which until now was stored, shown in the UI, exported, and read by *nothing*; values the **accounting** position not the shelf, since invoiced-not-shipped goods have had their revenue booked. Wired into P&L (opening/closing stock ⇒ correct COGS) and Balance Sheet (Stock-in-Hand asset). **#2 retained earnings:** balance sheet scoped profit to the current FY and nothing closed prior years, so it stopped balancing in year 2 — now derives `retainedEarningsBroughtForward` from all-time P&L entries + opening stock. **#3 TOCTOU:** all `getOrCreate*` ledger helpers were findFirst-then-create; P2002 in Postgres *poisons the whole posting transaction*, so an invoice would fail intermittently. Prisma `upsert` does **not** fix it (driver adapter resolves it as select-then-insert — integration test proves this); replaced with raw `INSERT ... ON CONFLICT (organizationId,name) DO UPDATE SET name=EXCLUDED.name RETURNING id`. **#4:** `services/inventory/movement-rules.ts` — direction table per movement type; a SALE naming only `toWarehouseId` used to *increase* stock. **#5:** `writeAudit` added to stock mutations. **#7:** real integration harness — `vitest.integration.config.ts` (pins DATABASE_URL to TEST_DATABASE_URL before app modules load), `tests/integration/{global-setup,setup,factories}.ts`, TRUNCATE between tests, `npm run test:integration`; CI now runs a Postgres service + `migrate deploy` + integration tests. **Migrations:** renamed my `10_org_payroll_settings` → `15_` (collided with `10_payment_receipt_voucher_fk`); verified all 16 apply clean on an empty DB with no drift. **Status: 556 unit + 29 integration green, tsc + lint clean.** Remaining: #6 cron tests, #8 verify 4 pages, #9 retrofit 3 mocked tests, #10 final sweep. Local test DB: `docker run -d --name accubook-test-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=accubook_test -p 55432:5432 postgres:16-alpine` | `79ca34e` |
| 2026-08-21 | **Swept every screen for dead controls and wired them to real behaviour.** Reported symptom: Inventory → Adjustment "New Adjustment" did nothing. A scanner over all `.tsx` (`<Button>` / `<DropdownMenuItem>` with no handler, not a trigger child) found **127 inert controls across 38 files**; all are now wired, leaving 3 justified (a `disabled` docs link, shadcn's calendar day button which gets `onClick` via `{...props}`, and a `disabled` "No branches found" placeholder). Along the way, data bugs the dead buttons were hiding: stock API ignored `?type=` so Adjustment/Movements rendered the *stock summary* through movement columns (added `movementType` filter); stock page fetched `?warehouses=true` (never implemented) so the Warehouses and Movements tabs were permanently empty and its Adjust Stock POST sent a payload the schema rejects; HR Employees read `{data,pagination}` as an array so the roster was always empty; hr/leaves mapped non-existent fields so its table was blank; Notifications never fetched at all; Departments' "create" showed a success toast without any request; Apply-for-Leave and Invite-User and Create-Role dialogs had unbound inputs and submits that only closed the dialog; report pages' period selectors changed state nothing read; balance sheet never rendered its own group breakdown; PO delete called a route that did not exist. **New API routes:** `departments/`(+`[departmentId]`), `leave-types/`, `employees/[employeeId]`, `purchase-orders/[orderId]`, `quotations/[quotationId]/convert`, `documents/send` (generic emailer via the existing Resend wrapper), `payroll/settings`, `users/reset-password`, `report-templates/`, `/api/profile` + `/api/profile/password`; PUT on `approvals` (workflow edit/toggle), POST/PATCH/DELETE on `roles`, PATCH+GET on `sales-orders/[orderId]`, `?view=transactions` on `ledgers/[ledgerId]`. **New shared frontend pieces:** `record-details-dialog.tsx`, `utils/print-document.ts` (iframe print → Save-as-PDF, incl. barcode images), `utils/export-report.ts`, `utils/report-period.ts`. **Migration 10** adds `Organization.payrollSettings` (JSON) so Payroll → Settings has a store. New routes registered in `API_RESOURCE_MAP` (route-coverage test) and use the `optional()` helper (validator test). 535 tests pass, tsc + lint (0 errors) + `next build` clean. Deliberately *not* built: 2FA — enabling it without enforcing a second factor at login would be a security control that controls nothing; that row now offers "Sign out everywhere" (real) instead. | _(working tree)_ |
...
- `npm test` — 624 unit tests. Fast, no database.
- `npm run test:integration` — 107 DB-backed tests. Needs a local `accubook_test`
  (`npm run test:integration:setup`). The one local failure in `harness.test.ts`
  (`inet_server_addr()` is null over a Unix socket) is an artifact of connecting
  without TCP; CI connects over TCP and it passes.
