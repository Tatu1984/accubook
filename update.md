...
- **Active workstreams:**
  ...
  - **WS7 (payroll) — month-end run posts to GL.** PF/ESI/PT/TDS/LOP compute helpers tested. `POST /payroll/post-month` builds a single JV (Dr Salaries & Wages net of LOP / Dr Employer PF / Dr Employer ESI / Cr Salaries Payable / Cr PF Payable / Cr ESI Payable / Cr PT Payable / Cr TDS Payable), links every included payslip to the voucher and moves them to PROCESSED. Drops zero lines (e.g. ESI not applicable). PAID step (when net leaves the bank) still pending. Salary structure UI / per-employee declarations UI also pending.
...
- **Last updated:** 2026-05-04 by Claude (commit pending — manufacturing issue/complete)
...
| Date | What | Commit |
|---|---|---|
| 2026-05-04 | **WS10 — manufacturing WO issue + complete.** New `POST /work-orders/[id]/issue` (BOM-scaled stock decrement, ISSUE movements, Dr WIP / Cr Stock-in-Hand JV, → IN_PROGRESS; structured shortage list on 400) and `POST /work-orders/[id]/complete` (GRN finished good with weighted-avg recompute, Dr Stock-in-Hand / Cr WIP, → COMPLETED). Scrap absorbed into FG cost. Seed adds "Stock-in-Hand" + "Work in Progress" ledgers. AuditAction enum +ISSUE/+COMPLETE. | _pending_ |
