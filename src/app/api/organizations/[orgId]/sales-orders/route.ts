...
const createSalesOrderSchema = z.object({
  ...
  expectedDate: optional(z.string()).transform((val) => val ? new Date(val) : undefined),
...
export const GET = withOrgAuth(async (request, { orgId }) => {
  try {
    ...
    logger.error({ err: error }, "Error fetching sales orders");
    return NextResponse.json(
...
export const POST = withOrgAuth(async (request, { orgId }) => {
  try {
    ...
    logger.error({ err: error }, "Error creating sales order");
    return NextResponse.json(
