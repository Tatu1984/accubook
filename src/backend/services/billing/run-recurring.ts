...
import { addFrequency, isFrequency, type Frequency } from "./recurring";
...
/**
 * Spawn one invoice per due recurring template for the given organization.
 *
 * One invoice per call per template — if a subscription is several cycles
 * behind, each tick catches up by one cycle. Mirrors the invoice POST's GST
 * split + composition handling + FY-scoped numbering.
 *
 * Called from the per-org `POST /recurring-invoices/run` route (manual
 * tick) and from the cross-org `POST /api/cron/run-recurring` (scheduled).
 * Idempotent within a single tick: advances `nextRunDate` inside the same
 * transaction that creates the invoice, so a re-run of the same minute
 * sees the advanced state and skips.
 */
export async function runRecurringForOrg(
  ...
  for (const row of dueRows) {
    try {
      ...
      // Invoice DATE = cycle date (nextRunDate); invoice NUMBER FY = the
      // run's actual execution FY, so back-dated catch-ups don't mint
      // numbers in a closed FY counter.
