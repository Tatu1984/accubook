...
- **Payments and receipts post to the GL atomically.** `src/backend/utils/posting.ts` exposes `applyLedgerEntries`, `getOrCreatePartyLedger`, `getOrCreateBankLedger`, `recomputeInvoiceStatus`, `recomputeBillStatus`. POST handlers wrap the entire posting in `prisma.$transaction`.
